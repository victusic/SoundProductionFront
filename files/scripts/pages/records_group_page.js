$(document).ready(function(){
	$.getJSON('../php/groups.php', function(data) {
		//console.log(data);
		...
		$('.listview').bind('click', function(){
			...
			sessionStorage.setItem('IdG', data[sessionStorage.getItem('GroupId')]['BandId']);
			...
			$('#list2_genre option[value='+ data[sessionStorage.getItem('GroupId')]['Genre'] +']').prop('selected', true);
			$('#list2_group').val(data[sessionStorage.getItem('GroupId')]['Description']);
			$('#list2_about_area').val(data[sessionStorage.getItem('GroupId')]['DescriptionAreas']);
			...
			$('#list2_spotify').val(data[sessionStorage.getItem('GroupId')]['SpotifyLink']);
			$('#list2_apple').val(data[sessionStorage.getItem('GroupId')]['AppleMusicLink']);
			$('#list2_yandex').val(data[sessionStorage.getItem('GroupId')]['YandexMusicLink']);
			$('#list2_deezer').val(data[sessionStorage.getItem('GroupId')]['DeezerLink']);
			$('#list2_youtube').val(data[sessionStorage.getItem('GroupId')]['YouTubeLink']);
			$('#list2_youtubemusic').val(data[sessionStorage.getItem('GroupId')]['YouTubeMusicLink']);
			...
			$('#l2_btns_pub').bind('click', function() {
				$.get('../php/pub-groups.php', {BandId: sessionStorage.getItem('IdG')}, function(data){
			...
			$('#l2_btns_shoot').bind('click', function() {
				$.get('../php/shoot-groups.php', {BandId: sessionStorage.getItem('IdG')}, function(data){
					location.reload();
				});
			});
			...
			$('#l2_btns_save').bind('click', function() {
				...
				if ($('#list2_genre').val() == "Выберите жанр") {
					wrng++;
				...
				if ($('#list2_spotify').val() == "") {
					$('#l2_spotify').css('color', 'var(--warningfont)');
				}
				if ($('#list2_apple').val() == "") {
					$('#l2_apple').css('color', 'var(--warningfont)');
				}
				if ($('#list2_deezer').val() == "") {
					$('#l2_deezer').css('color', 'var(--warningfont)');
				}
				if ($('#list2_yandex').val() == "") {
					$('#l2_yandex').css('color', 'var(--warningfont)');
				}
				if ($('#list2_youtube').val() == "") {
					$('#l2_youtube').css('color', 'var(--warningfont)');
				}
				if ($('#list2_youtubemusic').val() == "") {
					$('#l2_youtubemusic').css('color', 'var(--warningfont)');
				}
				if(wrng == 0){
					let dates = {
						BandId: data[sessionStorage.getItem('GroupId')]['BandId'],
						Name: $('#list2_inp_name').val(),
						Producer: sessionStorage.getItem('UserId'),
						Description: $('#list2_group').val(),
						DescriptionAreas: $('#list2_about_area').val(),
					};
					...
					if ($('#list2_spotify').val() == '') {
						dates['SpotifyLink'] = null
					}
					else{dates['SpotifyLink'] =  $('#list2_spotify').val();}
					if ($('#list2_apple').val() == '') {
						dates['AppleMusicLink'] = null
					}
					else{dates['AppleMusicLink'] =  $('#list2_apple').val();}
					if ($('#list2_deezer').val() == '') {
						dates['DeezerLink'] = null
					}
					else{dates['DeezerLink'] =  $('#list2_deezer').val();}
					if ($('#list2_yandex').val() == '') {
						dates['YandexMusicLink'] = null
					}
					else{dates['YandexMusicLink'] =  $('#list2_yandex').val();}
					if ($('#list2_youtubemusic').val() == '') {
						dates['YouTubeMusicLink'] = null
					}
					else{dates['YouTubeMusicLink'] =  $('#list2_youtubemusic').val();}
					if ($('#list2_youtube').val() == '') {
						dates['YouTubeLink'] = null
					}
					else{dates['YouTubeLink'] =  $('#list2_youtube').val();}

					dates['Phone'] = $('#list2_phone').val();
					dates['Mail'] = $('#list2_mail').val();
					dates['Genre'] = $('#list2_genre').val();
			...
			$('#l2_btns_del').bind('click', function() {
				if(sessionStorage.getItem('State') == 0 || sessionStorage.getItem('State') == 1){
					...
					$.get('../php/del-group.php', {text: sessionStorage.getItem('IdG')}, function(data){
						location.reload();
					});
				}
			});
		...
		$('#l3_btns_add').bind('click', function() {
			...
			if ($('#list3_spotify').val() == "") {
				$('#l3_spotify').css('color', 'var(--warningfont)');
			}
			if ($('#list3_apple').val() == "") {
				$('#l3_apple').css('color', 'var(--warningfont)');
			}
			if ($('#list3_deezer').val() == "") {
				$('#l3_deezer').css('color', 'var(--warningfont)');
			}
			if ($('#list3_yandex').val() == "") {
				$('#l3_yandex').css('color', 'var(--warningfont)');
			}
			if ($('#list3_youtube').val() == "") {
				$('#l3_youtube').css('color', 'var(--warningfont)');
			}
			if ($('#list3_youtubemusic').val() == "") {
				$('#l3_youtubemusic').css('color', 'var(--warningfont)');
			}
			if(wrng == 0){
				let dates = {
					//BandId: data[sessionStorage.getItem('GroupId')]['BandId'],
					...
					DescriptionAreas: $('#list3_about_area').val(),
				};
				...
				else{dates['SpotifyLink'] =  $('#list3_spotify').val();}
				if ($('#list3_apple').val() == '') {
					dates['AppleMusicLink'] = null;
				}
				else{dates['AppleMusicLink'] =  $('#list3_apple').val();}
				if ($('#list3_deezer').val() == '') {
					dates['DeezerLink'] = null;
				}
				else{dates['DeezerLink'] =  $('#list3_deezer').val();}
				if ($('#list3_yandex').val() == '') {
					dates['YandexMusicLink'] = null;
				}
				else{dates['YandexMusicLink'] =  $('#list3_yandex').val();}
				if ($('#list3_youtubemusic').val() == '') {
					dates['YouTubeMusicLink'] = null;
				}
				else{dates['YouTubeMusicLink'] =  $('#list3_youtubemusic').val();}
				if ($('#list3_youtube').val() == '') {
					dates['YouTubeLink'] = null;
				}
				else{dates['YouTubeLink'] =  $('#list3_youtube').val();}

				dates['Phone'] = $('#list3_phone').val();
				dates['Mail'] = $('#list3_mail').val();
				dates['Genre'] = $('#list3_genre').val();
