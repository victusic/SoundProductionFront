$(document).ready(function(){
	$.getJSON('../php/visit_groups.php', function(data) {
		var sec = 0;
		...
		$('#list2_end').bind('click', function(){
			...
			if (data.Count > 3) {
				$('#back').height(1065 + ((data.Count - 3) * 215));
				$('#base').height(1470 + ((data.Count - 3) * 215));
				$('#listr').height(980 + ((data.Count - 3) * 215));
				//$('#livi_plateS').height((data.Count * 315) + 30);
				$('#livi_plateS').height((data.Count * 315) + 30);
			}
		...
		$('#list2_show').bind('click', function(){
			...
			let slidesX = '<div class="slide1"></div>'

			if(slideY != 0){
				slidesX += '<div class="slide2"></div>';
			}
			
			if (countImage != 0){
				slidesX += '<div class="slide3"></div>';
			}

			$('.pr_base').html(slidesX);
			...
			if(data[sessionStorage.getItem('GroupId')]['PhotoLogo'] != null){
				...
				function animateName() {
					$(".pr_name").animate({opacity: "1",}, 1000)
					.animate({top: "180px", fontSize: "64px"}, 400);
					animateGenre();
				}
			}
